...
class Grid(plugins.Plugin):
    ...
    def check_inbox(self, agent):
        ...
        if self.unread_messages:
            ...
            logging.debug(
                f"[{self.__class__.__name__}] unread:%d total:%d"
    ...
    def check_handshakes(self, agent):
        logging.debug(f"[{self.__class__.__name__}] checking pcaps")
        config = agent.config()
        ...
        if num_new > 0:
            if self.options["report"]:
                ...
                logging.debug(
                    f"[{self.__class__.__name__}] self.options: %s" % self.options
                )
                logging.debug(
                    f"[{self.__class__.__name__}] exclude: %s"
                    % config["main"]["whitelist"]
                )
    ...
    def on_internet_available(self, agent):
        ...
        with self.lock:
            ...
            try:
                ...
                logging.error(
                    f"[{self.__class__.__name__}] error while checking inbox: %s" % e
                )
                logging.debug(e, exc_info=True)
            ...
            try:
                ...
                logging.error(
                    f"[{self.__class__.__name__}] error while checking pcaps: %s" % e
                )
                logging.debug(e, exc_info=True)
