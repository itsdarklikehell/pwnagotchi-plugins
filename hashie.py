...
class hashie(plugins.Plugin):
        ...
        with self.lock:
            ...
            if os.path.isfile(fullpathNoExt + '.2500'):
                handshake_status.append(
                    'Already have {}.2500 (EAPOL)'.format(name))
            elif self._writeEAPOL(filename):
                handshake_status.append(
                    'Created {}.2500 (EAPOL) from pcap'.format(name))
    ...
    def _writeEAPOL(self, fullpath):
        ...
        subprocess.getoutput(
            'hcxpcaptool -o {}.2500 {} >/dev/null 2>&1'.format(fullpathNoExt, fullpath))
        if os.path.isfile(fullpathNoExt + '.2500'):
            logging.debug(
                '[hashie] [+] EAPOL Success: {}.2500 created'.format(filename))
            return True
        else:
            return False
    ...
    def _writePMKID(self, fullpath, apJSON):
        ...
        filename = fullpath.split('/')[-1:][0].split('.')[0]
        subprocess.getoutput(
            'hcxpcaptool -k {}.16800 {} >/dev/null 2>&1'.format(fullpathNoExt, fullpath))
        if os.path.isfile(fullpathNoExt + '.16800'):
            logging.debug(
            ...
            if os.path.isfile(fullpathNoExt + '.16800'):
                if not self._repairPMKID(fullpath, apJSON):
                    logging.debug(
                        '[hashie] [-] PMKID Fail: {}.16800 could not be repaired'.format(filename))
                    return False
                else:
                    logging.debug(
    ...
    def _repairPMKID(self, fullpath, apJSON):
        ...
        if apJSON != "":
            clientString.append('{}:{}'.format(apJSON['mac'].replace(
                ':', ''), apJSON['hostname'].encode('hex')))
        else:
            # attempt to extract the AP's name via hcxpcaptool
            subprocess.getoutput(
                'hcxpcaptool -X /tmp/{} {} >/dev/null 2>&1'.format(filename, fullpath))
            if os.path.isfile('/tmp/' + filename):
                with open('/tmp/' + filename, 'r') as tempFileB:
                    temp = tempFileB.read().splitlines()
                    for line in temp:
                        clientString.append(line.split(
                            ':')[0] + ':' + line.split(':')[1].strip('\n').encode().hex())
                os.remove('/tmp/{}'.format(filename))
            # attempt to extract the AP's name via tcpdump
            tcpCatOut = subprocess.check_output(
                "tcpdump -ennr " + fullpath + " \"(type mgt subtype beacon) || (type mgt subtype probe-resp) || (type mgt subtype reassoc-resp) || (type mgt subtype assoc-req)\" 2>/dev/null | sed -E 's/.*BSSID:([0-9a-fA-F:]{17}).*\\((.*)\\).*/\\1\t\\2/g'", shell=True).decode('utf-8')
            if ":" in tcpCatOut:
                for i in tcpCatOut.split('\n'):
                    if ":" in i:
                        clientString.append(i.split('\t')[0].replace(
                            ':', '') + ':' + i.split('\t')[1].strip('\n').encode().hex())
        if clientString:
                ...
                if line.split(':')[0] == hashString.split(':')[1]:
                    ...
                    if (len(hashString.split(':')) == 4) and not (hashString.endswith(':')):
                        with open(fullpath.split('.')[0] + '.16800', 'w') as tempFileC:
                            logging.debug('[hashie] Repaired: {} ({})'.format(
                                filename, hashString))
    ...
    def _process_stale_pcaps(self, handshake_dir):
        handshakes_list = [os.path.join(handshake_dir, filename) for filename in os.listdir(
            handshake_dir) if filename.endswith('.pcap')]
        failed_jobs = []
        successful_jobs = []
        lonely_pcaps = []
        for num, handshake in enumerate(handshakes_list):
            ...
            if not os.path.isfile(fullpathNoExt + '.16800'):  # if no 16800, try
                if self._writePMKID(handshake, ""):
                    ...
                    # if no 16800 AND no 2500
                    if not os.path.isfile(fullpathNoExt + '.2500'):
                        lonely_pcaps.append(handshake)
                        logging.debug(
                            '[hashie] Batch job: added {} to lonely list'.format(pcapFileName))
            # report progress every 50, or when done
            if ((num + 1) % 50 == 0) or (num + 1 == len(handshakes_list)):
                logging.info('[hashie] Batch job: {}/{} done ({} fails)'.format(
                    num + 1, len(handshakes_list), len(lonely_pcaps)))
        if successful_jobs:
            logging.info('[hashie] Batch job: {} new handshake files created'.format(
                len(successful_jobs)))
        if lonely_pcaps:
            logging.info('[hashie] Batch job: {} networks without enough packets to create a hash'.format(
                len(lonely_pcaps)))
        ...
        for pcapFile in lonely_pcaps:
            ...
            if os.path.isfile(fullpathNoExt + '.gps.json'):
                with open(fullpathNoExt + '.gps.json', 'r') as tempFileA:
                    ...
                    locations.append(
                        filename + ',' + str(data['Latitude']) + ',' + str(data['Longitude']) + ',50')
            elif os.path.isfile(fullpathNoExt + '.geo.json'):
                with open(fullpathNoExt + '.geo.json', 'r') as tempFileB:
                    data = json.load(tempFileB)
                    locations.append(filename + ',' + str(data['location']['lat']) + ',' + str(
                        data['location']['lng']) + ',' + str(data['accuracy']))
                ...
                with open(fullpathNoExt + '.paw-gps.json', 'r') as tempFileC:
                    ...
                    locations.append(
                        filename + ',' + str(data['lat']) + ',' + str(data['long']) + ',50')
        if locations:
            with open('/root/locations.csv', 'w') as tempFileD:
                for loc in locations:
                    tempFileD.write(loc + '\n')
            logging.info(
                '[hashie] Used {} GPS/GEO files to find lonely networks, load /root/locations.csv into a mapping app and go say hi!'.format(len(locations)))
