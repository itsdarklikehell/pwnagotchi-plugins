...
def grab_bssid(file):
    try:
        process = Popen(['tshark', '-r', config['local_dir'] + file, '-T', 'fields', '-e', 'wlan.bssid'], stdout=PIPE, stderr=PIPE)
...
def convert_file(file):
    try:
        ...
        process = Popen(['hcxpcapngtool', '-o', f'./pmkid/{file.replace(".pcap", "")}.pmkid', config['local_dir'] + file], stdout=PIPE, stderr=PIPE)
        ...
        if 'PMKID(s) written' in stderr.decode():
            ...
            process = Popen(['hcxpcapngtool', '-o', f'./hccapx/{file.replace(".pcap", "")}.hccapx', config['local_dir'] + file], stdout=PIPE, stderr=PIPE)
            _, stderr = process.communicate()
...
def main():
    ...
    parser.add_argument('-r', '--remove', action='store_true', help='delete handshake files after processing')
