...
class NetPos_ng(plugins.Plugin):
    ...
    __name__ = "NetPos_ng"
    __help__ = """Saves a json file with the access points with more signal
                         whenever a handshake is captured.
                         When internet is available the files are converted in geo locations
                         using Mozilla LocationService """
    __dependencies__ = {
    ...
    def on_loaded(self):
        if "api_key" not in self.options or (
            ...
            logging.error(
                f"[{self.__class__.__name__}] api_key isn't set. Can't use mozilla's api."
            )
            return
    ...
    def on_internet_available(self, agent):
        ...
        with self.lock:
            if self.ready:
                ...
                if new_np_files:
                    logging.debug(
                        f"[{self.__class__.__name__}] Found %d new net-pos files. Fetching positions ...",
                        len(new_np_files),
                    )
                    display.set(
                        "status",
                    ...
                    for idx, np_file in enumerate(new_np_files):
                        ...
                        try:
                            ...
                            logging.error(
                                f"[{self.__class__.__name__}] %s - JSONDecodeError: %s, removing it...",
                                np_file,
                                js_e,
                            )
                            os.remove(np_file)
                            ...
                            logging.error(
                                f"[{self.__class__.__name__}] %s - OSError: %s",
                                np_file,
                                os_e,
                            )
                            self.skip += np_file
    ...
    def on_handshake(self, agent, filename, access_point, client_station):
        ...
        logging.debug(
            f"[{self.__class__.__name__}] Saving net-location to %s", netpos_filename
        )
        ...
        try:
            ...
            logging.error(f"[{self.__class__.__name__}] %s", os_e)
