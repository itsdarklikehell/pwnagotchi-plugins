...
class Wardriver(plugins.Plugin):
    ...
    __name__ = "Wardriver"
    __help__ = "A wardriving plugin for pwnagotchi. Saves all networks seen and uploads data to WiGLE once internet is available"
    __dependencies__ = {
        "apt": ["none"],
        "pip": ["scapy"],
    }
    ...
    def on_loaded(self):
        ...
        if "ui" in self.options:
            self.__ui_enabled = (
                self.options["ui"]["enabled"]
                if "enabled" in self.options["ui"]
                else False
            )
            try:
                self.__ui_position = (
                    self.options["ui"]["position"]["x"],
                    self.options["ui"]["position"]["y"],
                )
            except Exception:
        ...
        if "wigle" in self.options:
            self.__wigle_enabled = (
                self.options["wigle"]["enabled"]
                if "enabled" in self.options["wigle"]
                else False
            )
            self.__wigle_api_key = (
                self.options["wigle"]["api_key"]
                if "api_key" in self.options["wigle"]
                else None
            )
            self.__wigle_donate = (
                self.options["wigle"]["donate"]
                if "donate" in self.options["wigle"]
                else False
            )
            if self.__wigle_enabled and (
                not self.__wigle_api_key or self.__wigle_api_key == ""
            ):
                logging.error(
                    f"[{self.__class__.__name__}] Wigle enabled but no api key provided!"
                )
        ...
        logging.info(
            f"[{self.__class__.__name__}] Saving session files inside {self.__csv_path}"
        )

        if self.__wigle_enabled:
            logging.info(
                f"[{self.__class__.__name__}] Previous sessions will be uploaded to WiGLE once internet is available"
            )
        ...
        if len(self.__whitelist) > 0:
            logging.info(
                f"[{self.__class__.__name__}] Ignoring {len(self.__whitelist)} networks"
            )

    def on_ui_setup(self, ui):
        if self.__ui_enabled:
            logging.info(f"[{self.__class__.__name__}] Adding status text to ui")
            ui.add_element(
                "wardriver",
                LabeledValue(
                    color=BLACK,
    ...
    def __clean_csv_directory(self):
        ...
        """
        sessions = [
            os.path.join(self.__csv_path, file) for file in os.listdir(self.__csv_path)
        ]
        if len(sessions) > 0:
            logging.info(f"[{self.__class__.__name__}] Removing empty session files")
                ...
                try:
                    with open(file, "r") as session_file:
                        ...
                        if len(session_data.split("\n")) <= 3:
                            os.remove(file)
                            logging.info(
                                f"[{self.__class__.__name__}] File {file} removed"
                            )
                except Exception as e:
                    logging.error(
                        f"[{self.__class__.__name__}] Error while processing {file}"
                    )
    ...
    def __new_wardriving_session(self):
        ...
        logging.info(
            f"[{self.__class__.__name__}] Initializing new session file {session_file}"
        )
        ...
        try:
            with open(self.__session_file, "w") as csv:
                ...
                csv.write(
                    f'{preheader_data["file_format"]},{preheader_data["app_release"]},{preheader_data["model"]},{preheader_data["release"]},{preheader_data["device"]},{preheader_data["display"]},{preheader_data["board"]},{preheader_data["brand"]}\n'
                )
                # CSV header
                csv.write(
                ...
                logging.info(
                    f"[{self.__class__.__name__}] Session file initialized. Ready to wardrive!"
                )
                self.ready = True
        except Exception as e:
            logging.critical(
                f"[{self.__class__.__name__}] Error while creating session file! {e}"
            )
    ...
    def __filter_whitelist_aps(self, unfiltered_aps):
        ...
        """
        filtered_aps = [
            ap for ap in unfiltered_aps if ap["hostname"] not in self.__whitelist
        ]
        return filtered_aps
    ...
    def __filter_reported_aps(self, unfiltered_aps):
        ...
        filtered_aps = [
            ...
            if (ap["mac"], ap["hostname"]) not in self.__session_reported
        ]
        return filtered_aps
    ...
    def __ap_to_csv(self, ap, coordinates):
        ...
        )  # Use now() instead of bettercap firstSeen
        channel = ap["channel"]
        rssi = ap["rssi"]
        ...
        with open(self.__session_file, "+a") as file:
            for ap in aps:
                try:
                    ...
                    logging.error(
                        f"[{self.__class__.__name__}] Error while logging to csv file: {e}"
                    )
    ...
    def on_unfiltered_ap_list(self, agent, aps):
        ...
        if (
            not self.ready
        ):  # it is ready once the session file has been initialized with pre-header and header
            logging.error(
                f"[{self.__class__.__name__}] Plugin not ready... skip wardriving log"
            )
        ...
        if gps_data and all(
            ...
            if len(filtered_aps) > 0:
                logging.info(
                    f"[{self.__class__.__name__}] Discovered {len(filtered_aps)} new networks"
                )
                self.__update_csv_file(filtered_aps, coordinates)
        else:
            logging.warning(
                f"[{self.__class__.__name__}] GPS not available... skip wardriving log"
            )
    ...
    def on_internet_available(self, agent):
        if self.__wigle_enabled and not self.__lock.locked():
            with self.__lock:
                sessions_to_upload = [
                    os.path.join(self.__csv_path, file)
                    for file in os.listdir(self.__csv_path)
                    if os.path.isfile(os.path.join(self.__csv_path, file))
                    and file.endswith(".csv")
                    and file != os.path.basename(self.__session_file)
                    and not "uploaded" in file
                ]
                if len(sessions_to_upload) > 0:
                    logging.info(
                        f"[{self.__class__.__name__}] Uploading previous sessions on WiGLE ({len(sessions_to_upload)} sessions) - current session will not be uploaded"
                    )
                    headers = {
                    ...
                    for file in sessions_to_upload:
                        try:
                            ...
                            logging.error(
                                f"[{self.__class__.__name__}] Failed reading file {file}: {e}"
                            )
                            continue
                        ...
                        try:
                            ...
                            try:
                                ...
                                logging.critical(
                                    f"[{self.__class__.__name__}] Error while marking {file} as uploaded {e}"
                                )
                        except Exception as e:
                            logging.error(
                                f"[{self.__class__.__name__}] Failed uploading file {file}: {e}"
                            )
                            continue
