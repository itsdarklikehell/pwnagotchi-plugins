...
class quick_rides_to_jail(plugins.Plugin):
    ...
    def _device_in_monitor_mode(self, device_name):
        ...
        try:
            ...
            device_type = ""
            logging.error(
                f"[{self.__class__.__name__}] Exception while opening and reading network device: %s",
                e,
            )
    ...
    def _do_the_illegal_thing(self, handshakes_path):
        if OPTIONS["interface"] not in _get_network_interfaces():
            ...
            return
        logging.info(
            f"[{self.__class__.__name__}] Found desired interface in list of local interfaces."
        )
