...
class discohash(plugins.Plugin):
    ...
    __version__ = "1.1.0"
    __license__ = "GPL3"
    __description__ = "DiscoHash extracts hashes from pcaps (hashcat mode 22000) using hcxpcapngtool, analyses the hash using hcxhashtool and posts the output to Discord along with any obtained GPS coordinates."
    __name__ = "discohash"
            ...
            if not os.path.isfile(fullpathNoExt + ".22000"):
                if self.write_hash(handshake):
                    ...
                    if not os.path.isfile(fullpathNoExt + ".22000"):
                        ...
                        logging.debug(
                            "[*] DiscoHash Batch job: added {} to lonely list".format(
                                pcapFileName
                            )
                        )
            if ((num + 1) % 10 == 0) or (num + 1 == len(handshakes_list)):
                logging.debug(
                    "[*] DiscoHash Batch job: {}/{} done ({} fails)".format(
                        num + 1, len(handshakes_list), len(lonely_pcaps)
                    )
                )
        if successful_jobs:
            logging.debug(
                "[*] DiscoHash Batch job: {} new handshake files created".format(
                    len(successful_jobs)
                )
            )
        if lonely_pcaps:
            logging.debug(
                "[*] DiscoHash Batch job: {} networks without enough packets to create a hash".format(
                    len(lonely_pcaps)
                )
            )
    ...
    def post_hash(self, fullpathNoExt):
        ...
        try:
            data = {
                "embeds": [
                    {
                        "title": "(ᵔ◡◡ᵔ) {} sniffed a new hash!".format(
                            pwnagotchi.name()
                        ),
                        "color": 289968,
