...
class GdriveSync(plugins.Plugin):
    ...
    __name__ = "GdriveSync"
    __help__ = = "A plugin to backup various pwnagotchi files and folders to Google Drive. Once every hour from loading plugin."
    __dependencies__ = {
        "apt": ["none"],
        "pip": ["scapy"],
    }
    ...
    def on_loaded(self):
        ...
        if os.stat("/root/client_secrets.json").st_size == 0:
            logging.error(
                f"[{self.__class__.__name__}] /root/client_secrets.json is empty. Please RTFM!"
            )
            return
        ...
        try:
            ...
            if not self.backup:
                ...
                if not backup_folder_file_list:
                    ...
                    logging.warning(f"[{self.__class__.__name__}] No files found in the folder with ID {root_file_list} and {pwnagotchi_file_list}")
                ...
                if zip_file_id:
                    ...
                    logging.info(f"[{self.__class__.__name__}] Downloaded backup.zip from Google Drive")
    ...
    def create_folder_if_not_exists(self, backup_folder_name):
        ...
        if backup_folder_id is None:
            ...
            logging.info(
                f"[{self.__class__.__name__}] Created folder '{backup_folder_name}' with ID: {backup_folder_id}"
            )
    ...
    def on_handshake(self, agent):
        ...
        with self.lock:
            ...
            logging.info(f"[{self.__class__.__name__}] new handshake captured, backing up to gdrive")
            if self.options["backupfiles"] is not None:
    ...
    def backup_files(self, paths, dest_path):
        for src_path in paths:
            try:
                ...
                logging.error(f"[{self.__class__.__name__}] Error during backup_path: {e}")
    ...
    def upload_to_gdrive(self, backup_path, gdrive_folder):
        try:
            ...
            logging.error(f"[{self.__class__.__name__}] Error during upload_to_gdrive: {e}")
