...
class View(object):
    def __init__(self, config, impl, state=None):
        ...
        colormode = (
            "1"
            if not "colormode" in self._config["ui"]
            else self._config["ui"]["colormode"]
        )
        if "foregroundcolor" in self._config["ui"]:
            pwnagotchi.ui.view.BLACK = self._config["ui"]["foregroundcolor"]
        if "backgroundcolor" in self._config["ui"]:
            pwnagotchi.ui.view.WHITE = self._config["ui"]["backgroundcolor"]
    ...
    def get_default_backgroundcolor(self):
        return (
            WHITE
            if not "backgroundcolor" in self._config["ui"]
            else self._config["ui"]["backgroundcolor"]
        )
    ...
    def get_default_foregroundcolor(self):
        return (
            BLACK
            if not "foregroundcolor" in self._config["ui"]
            else self._config["ui"]["foregroundcolor"]
        )
