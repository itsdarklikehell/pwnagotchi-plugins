    ...
    def __init__(self, config, impl, state=None):
        ...
        self._state = State(
            state={
                ...
                "face": Text(
                    value=faces.SLEEP,
                    position=th_face["position"],
                    color=th_face["color"],
                    font=getattr(fonts, th_face["font"]),
                ),
                "friend_face": Text(
                    value=None,
                    position=th_fface["position"],
                    font=getattr(fonts, th_fface["font"]),
                    color=th_fface["color"],
                ),
                "friend_name": Text(
                    value=None,
                    position=th_fname["position"],
                    font=getattr(fonts, th_fname["font"]),
                    color=th_fname["color"],
                ),
                "name": Text(
                    value="%s>" % "pwnagotchi",
                    position=th_name["position"],
                    color=th_name["color"],
                    font=getattr(fonts, th_name["font"]),
                ),
                ...
                "shakes": LabeledValue(
                    ...
                    value=th_sh["value"],
                    color=th_sh["color"],
                    position=th_sh["position"],
                    label_font=getattr(fonts, th_sh["label_font"]),
                    text_font=getattr(fonts, th_sh["text_font"]),
                ),
                "mode": Text(
                    value="AUTO",
                    position=th_mode["position"],
                    font=getattr(fonts, th_mode["font"]),
                    color=th_mode["color"],
                ),
            }
        )
        ...
        if config["ui"]["fps"] > 0.0:
            ...
            logging.warn("ui.fps is 0, the display will only update for major changes")
        ...
        while True:
            try:
                ...
                self.set(
                    "name",
                    (
                        name.rstrip(th_opt["cursor"]).strip()
                        if th_opt["cursor"] in name
                        else (name + " " + th_opt["cursor"])
                    ),
                )
                self.update()
            except Exception as e:
                logging.warn("non fatal error while updating view: %s" % e)
    ...
    def update(self, force=False, new_data={}):
        ...
        with self._lock:
            ...
            if force or len(changes):
                ...
                copy_state = list(
                    state.items()
                )  # way to avoid [WARNING] non fatal error while updating view: dictionary changed size during iteration
                ...
                bg = Image.open(
                    "%s/fancygotchi/img/%s"
                    % (pwnagotchi.fancy_root, th_opt["bg_image"])
                )
                ...
                if th_opt["color_web"] == "full" or th_opt["color_display"] == "full":
                    bga = bg.convert("RGBA")
                    # logging.warn('A full color image is created')
                    for item in datas:
                        # logging.info(item)
                    ...
                    rgba_im = Image.new(
                        "RGBA", (self._width, self._height), (255, 255, 255, 0)
                    )
                ...
                if th_opt["color_web"] != "full" or th_opt["color_display"] != "full":
                    ...
                    if th_opt["color_web"] == "2" or th_opt["color_display"] == "2":
                        bga = bg.convert("RGBA")
                        # logging.warn('A 1bit image is created')
                        rgba_im_2 = Image.new(
                            "RGBA", (self._width, self._height), (255, 255, 255, 0)
                        )
                    ...
                    if th_opt["color_web"] == "3" or th_opt["color_display"] == "3":
                        bga = bg.convert("RGBA")
                        # logging.warn('A grayscale image is created')
                        rgba_im_3 = Image.new(
                            "RGBA", (self._width, self._height), (255, 255, 255, 0)
                        )
