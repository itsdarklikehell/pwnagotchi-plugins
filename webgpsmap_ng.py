...
class Webgpsmap(plugins.Plugin):
    ...
    __name__ = "Webgpsmap_ng"
    __help__ = "a plugin for pwnagotchi that shows a openstreetmap with positions of ap-handshakes in your webbrowser"
    __dependencies__ = {
        "apt": ["none"],
        "pip": ["scapy"],
    }
    ...
    def on_webhook(self, path, request):
        ...
        if not self.ready:
            try:
                ...
                logging.error(
                    f"[{self.__class__.__name__}] on_webhook NOT_READY error: {error}"
                )
                return
        else:
            if request.method == "GET":
                if path == "/" or not path:
                    ...
                    try:
                        ...
                        logging.error(
                            f"[{self.__class__.__name__}] on_webhook / error: {error}"
                        )
                        return
                    response_status = 200
                    response_mimetype = "application/xhtml+xml"
                    ...
                    try:
                        ...
                        logging.error(
                            f"[{self.__class__.__name__}] on_webhook all error: {error}"
                        )
                        return
                    ...
                    try:
                        ...
                        logging.error(
                            f"[{self.__class__.__name__}] on_webhook offlinemap: error: {error}"
                        )
                        return
        ...
        try:
            ...
            logging.error(
                f"[{self.__class__.__name__}] on_webhook CREATING_RESPONSE error: {error}"
            )
            return
    ...
    def load_gps_from_dir(self, gpsdir, newest_only=False):
        ...
        logging.info(f"[{self.__class__.__name__}] scanning {handshake_dir}")
        ...
        for filename_pcap in all_pcap_files:
            ...
            logging.debug(f"[{self.__class__.__name__}] search for .geo.json")
            ...
            logging.debug(
                f"[{self.__class__.__name__}] end search for position data files and use {filename_position}"
            )
        ...
        for pos_file in all_geo_or_gps_files:
            try:
                ...
                self.SKIP += pos_file
                logging.error(
                    f"[{self.__class__.__name__}] JSONDecodeError in: {pos_file} - error: {error}"
                )
                continue
            except ValueError as error:
                self.SKIP += pos_file
                logging.error(
                    f"[{self.__class__.__name__}] ValueError: {pos_file} - error: {error}"
                )
                continue
    ...
    def get_html(self):
        ...
        try:
            ...
            logging.error(
                f"[{self.__class__.__name__}] error loading template file {template_file} - error: {error}"
            )
        return html_data
...
class PositionFile:
    ...
    def __init__(self, path):
        ...
        try:
            logging.debug(f"[{self.__class__.__name__}] loading {path}")
            with open(path, "r") as json_file:
                self._json = json.load(json_file)
            logging.debug(f"[{self.__class__.__name__}] loaded {path}")
    ...
    def password(self):
        ...
        if os.path.isfile(password_file_path):
            try:
                ...
                logging.error(
                    f"[{self.__class__.__name__}] OS error loading password: {password_file_path} - error: {format(error)}"
                )
            except:
                logging.error(
                    f"[{self.__class__.__name__}] Unexpected error loading password: {password_file_path} - error: {sys.exc_info()[0]}"
                )
                raise
        return return_pass
