...
class Tweak_View(plugins.Plugin):
    ...
    def show_tweaks(self, request):
        ...
        for tw, val in self._tweaks.items():
            res += (
                '<li><input type=checkbox name=delete_me id="%s" value="%s"> %s: %s\n'
                % (tw, tw, tw, repr(val))
            )
    ...
    def dump_item(self, name, item, prefix=""):
        ...
        res = ""
        if type(item) is int:
            res += '%s: <input type=text name="%s%s" value="%s">' % (
                name,
                prefix,
                name,
                item,
                ...
                for key in dir(item):
                    ...
                    if key.startswith("__"):
                        ...
                        res += (
                            ...
                            % (name, prefix, name, html.escape(",".join(map(str, val))))
                        )
                    elif key in ("label", "label_spacing"):
                        res += (
                            '<li>%s.%s: <input type=text name="%s.%s.%s" value="%s"><br>'
                            % (
                                name,
                                html.escape(key),
                                prefix,
                        ...
                        for l, f in self.myFonts.items():
                            if val == f:
                                res += '  <option value="%s" selected>%s</option>' % (
                                    l,
                                    l,
                                )
                            else:
                        ...
                        res += '<li>%s["%s"].%s = %s\n' % (
                            ...
                            name,
                            html.escape(key),
                            html.escape("<" + str(type(val).__name__) + ">"),
                        )
                ...
                for key in dir(item):
                    ...
                    if key.startswith("__"):
                        res += "<li>%s%s.%s = %s\n" % (
                            ...
                            name,
                            key,
                            html.escape(repr(getattr(item, key))),
                        )
                        pass
                    else:
                        self._logger.debug(
                            "%s>>> %s:%s"
                            % (prefix, key, html.escape(str(type(getattr(item, key)))))
                        )
                        res += "<li>%s%s.%s = %s %s\n" % (
                            prefix,
                            name,
                        ...
                        if "font" in key[2]:
                            ...
                            for i in range(len(oldval)):
                                if oldval[i] != new_xy[i]:
                                    res += "<li>-%s.%s != %s, %s" % (
                                        key[1],
                                        key[2],
                                        html.escape(repr(new_xy)),
                                        html.escape(repr(oldval)),
                                    )
                                    res += "<li>-%s.%s != %s, %s" % (
                                        key[1],
                                        key[2],
                            ...
                            if oldval != str(val):
                                res += "<li>S%s.%s != %s, %s (%s)" % (
                                    ...
                                    key[2],
                                    html.escape(str(val)),
                                    html.escape(str(oldval)),
                                    html.escape(str(type(val))),
                                )
                                self._tweaks[k] = val
                                changed = True
                    ...
                    if "delete_me" in request.form:
                        ...
                        if changed:
                            try:
                                ...
                                ret += "<li><b>Unable to save settings:</b> %s" % repr(
                                    err
                                )
    ...
    def on_ready(self, agent):
        ...
        self._conf_file = (
            ...
            else "/etc/pwnagotchi/tweak_view.json"
        )
            ...
            for tag, value in self._untweak.items():
                ...
                if key in dir(ui._state._state[element]):
                    if key == "xy":
                        ...
                        self._logger.info(
                            "Reverted %s xy to %s"
                            % (element, repr(ui._state._state[element].xy))
                        )
                    else:
                        try:
                            ...
                            self._logger.warning(
                                "ui unload revert %s: %s, %s"
                                % (tag, repr(err), repr(ui))
                            )
        except Exception as err:
                    ...
                    if element in state and key in dir(state[element]):
                        ...
                        if key == "xy":
                            ...
                            if ui._state._state[element].xy != new_xy:
                                ...
                                self._logger.info(
                                    "Updated xy to %s"
                                    % repr(ui._state._state[element].xy)
                                )
                        elif key == "font":
                            ...
                            if value in self.myFonts:
                                ui._state._state[element].text_font = self.myFonts[
                                    value
                                ]
                        elif key == "label_font":
                            if value in self.myFonts:
                                ui._state._state[element].label_font = self.myFonts[
                                    value
                                ]
                        elif key == "label":
