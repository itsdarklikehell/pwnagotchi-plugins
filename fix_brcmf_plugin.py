...
class Fix_BRCMF(plugins.Plugin):
    ...
    def on_ready(self, agent):
        try:
            ...
            if len(self.pattern.findall(last_lines)) >= 3:
                ...
                logging.info(
                    "[FixBRCMF] Blind-Bug detected. Restarting.\n%s" % repr(last_lines)
                )
                try:
        ...
        if re.search("wifi error while hopping to channel", event["data"]["Message"]):
            ...
            try:
                ...
                if result["success"]:
                    ...
                    logging.warning(
                        "[FixBRCMF] wifi.recon flip: FAILED: %s" % repr(result)
                    )
            except Exception as err:
    ...
    def on_epoch(self, agent, epoch, epoch_data):
        ...
        if time.time() - self.LASTTRY > 180:
            ...
            if len(self.pattern.findall(last_lines)) >= 3:
                logging.info(
                    "[FixBRCMF]**** Should trigger a reload of the mon0 device:\n%s"
                    % last_lines
                )
                if hasattr(agent, "view"):
                    display = agent.view()
        ...
        if self.isReloadingMon0 and (time.time() - self.LASTTRY) < 180:
            ...
            try:
                ...
                if "success" in result:
                    self.logPrintView(
                        "info",
                        "[FixBRCMF] wifi.recon off: %s!" % repr(result),
                        display,
                    ...
                    self.logPrintView(
                        "warning",
                        "[FixBRCMF] wifi.recon off: FAILED: %s" % repr(result),
                        display,
                        {
            ...
            try:
                cmd_output = subprocess.check_output(
                    "sudo ifconfig mon0 down && sudo iw dev mon0 del", shell=True
                )
                self._status = "dn"
                self.logPrintView(
                    "info",
                    "[FixBRCMF] mon0 down and deleted: %s" % cmd_output,
                    display,
                    {"status": "mon0 d-d-d-down!", "face": faces.BORED},
                )
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            self.logPrintView(
                                "info",
                                "[FixBRCMF interface add mon0] worked #%d: %s"
                                % (tries, cmd_output),
                            )
                            self._status = "up"
                            time.sleep(tries + 5)
                            try:
                                ...
                                if "success" in result:
                                    logging.info(
                                        "[FixBRCMF set wifi.interface mon0] worked: %s"
                                        % repr(result)
                                    )
                                    self._status = ""
                                    ...
                                    logging.info(
                                        "[FixBRCMF set wifi.interfaceface mon0] failed? %s"
                                        % repr(result)
                                    )
                            except Exception as err:
                                logging.info(
                                    "[FixBRCMF set wifi.interface mon0] except: %s"
                                    % (repr(result), repr(err))
                                )
                        except Exception as cerr:  #
                            if not display:
                                print(
                                    "failed loading mon0 attempt #%d: %s"
                                    % (tries, repr(cerr))
                                )
                        ...
                        logging.error(
                            "[FixBRCMF] Failed reloading brcmfmac %s" % repr(err)
                        )
                    ...
                    if not display:
                        print("[FixBRCMF #%d modprobe -r] %s" % (tries, repr(nope)))
                    pass
            ...
            try:
                ...
                if "success" in result:  # and result["success"] is True:
                    ...
                    logging.error(
                        "[FixBRCMF] wifi.recon did not start up: %s" % repr(result)
                    )
