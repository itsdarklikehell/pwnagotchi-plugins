...
class Wardriver_ng(plugins.Plugin):
    ...
    def __clean_csv_directory(self):
        ...
        if len(sessions) > 0:
            logging.info(
                f'[{self.__class__.__name__}] Removing empty session files')
    ...
    def __filter_reported_aps(self, unfiltered_aps):
        ...
        '''
        filtered_aps = [ap for ap in unfiltered_aps if (
            ap['mac'], ap['hostname']) not in self.__session_reported]
        return filtered_aps
    ...
    def __ap_to_csv(self, ap, coordinates):
        ...
        first_timestamp_seen = datetime.now().astimezone(timezone.utc).strftime(
            '%Y-%m-%d %H:%M:%S')  # Use now() instead of bettercap firstSeen
        channel = ap['channel']
        rssi = ap['rssi']
    ...
    def on_internet_available(self, agent):
        if self.__wigle_enabled and not self.__lock.locked():
            with self.__lock:
                sessions_to_upload = [os.path.join(self.__csv_path, file) for file in os.listdir(self.__csv_path) if os.path.isfile(os.path.join(
                    self.__csv_path, file)) and file.endswith(".csv") and file != os.path.basename(self.__session_file) and not 'uploaded' in file]
