...
class PwnMothership(plugins.Plugin):
    ...
    def _post_to_mothership(self):
        ...
        with self.lock:
            ...
            try:
                ...
                logging.debug(
                    "Pwnmothership: Successfully uploaded %s",
                )
    ...
    def _return_json(self, as_string=True):
        if self.DISPLAY is None:
            return jsonify(
                {"initialised": "false", "fingerprint": self.AGENT.fingerprint()}
            )
        ...
        pwnd_tot = utils.total_unique_handshakes(
            self.AGENT.config()["bettercap"]["handshakes"]
        )
