...
def on_ready(self, agent):
    ...
    try:
        ...
        for pcap_file in new_pcap_files:
            logging.info(
                "[thePolice] Running uncracked pcap through aircrack: %s" % (pcap_file)
            )
            try:
...
def _do_crack(self, agent, filename):
    ...
    logging.info(
        "[thePolice] Confirmed handshakes captured for BSSID: %s",
...
def _reconfigure_wpa_supplicant(self):
    try:
        ...
        if result.strip() == "OK":
            logging.info(
                "[thePolice] Successfully updated wpa_supplicant for {}.".format(
                    OPTIONS["interface"]
                )
            )
            return
        logging.info(
            "[thePolice] Failed to update wpa_supplicant for {}.".format(
                OPTIONS["interface"]
            )
        )
...
def _add_pwnd_networks_to_wpa_supplicant(self, handshakes_path):
    ...
    if updated_count > 0:
        logging.info(
            "[thePolice] Congratulations! You added {} new access points to your wpa_supplicant.conf.".format(
                updated_count
            )
        )
...
def _device_in_monitor_mode(self, device_name):
    ...
    try:
        ...
        device_type = ""
        logging.error(
            "[thePolice] Exception while opening and reading network device: %s", e
        )
...
def _do_the_illegal_thing(self, handshakes_path):
    if OPTIONS["interface"] not in _get_network_interfaces():
        logging.info(
            "[thePolice] Could not find desired interface in list of local interfaces."
        )
        return
