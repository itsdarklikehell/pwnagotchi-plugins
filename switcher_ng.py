...
class Switcher(plugins.Plugin):
    ...
    def on_loaded(self):
        if "tasks" in self.options and self.options["tasks"]:
            ...
            logging.debug(f"[{self.__class__.__name__}] No tasks found...")
            return

        logging.info(f"[{self.__class__.__name__}] is loaded.")

        # create hooks
        logging.debug(f"[{self.__class__.__name__}] creating hooks...")
        methods = [
            "webhook",
            "internet_available",
            "ui_setup",
