...
class GdriveSync(plugins.Plugin):
        ...
        try:
            ...
            if not self.backup:
                ...
                if zip_file_id:
                    ...
                    logging.info(
                        f"[{self.__class__.__name__}] Downloaded backup.zip from Google Drive"
                    )
    ...
    def backup_files(self, paths, dest_path):
        for src_path in paths:
            try:
                ...
                logging.error(
                    f"[{self.__class__.__name__}] Error during backup_path: {e}"
                )
    ...
    def upload_to_gdrive(self, backup_path, gdrive_folder):
        try:
            ...
            logging.error(
                f"[{self.__class__.__name__}] Error during upload_to_gdrive: {e}"
            )
