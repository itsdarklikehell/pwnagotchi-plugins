...
def install(config):
    ...
    logging.info('[FANCYTOOLS] starting ' +
...
def update(online):
    ...
    logging.info('[FANCYGOTCHI] %s/fancygotchi.py ====> %s/fancygotchi.py' %
                 (path_upd, FANCY_ROOT))
    shutil.copyfile('%s/fancygotchi.py' % (path_upd),
                    '%s/fancygotchi.py' % (FANCY_ROOT))
...
def verify_fancygotchi_status(config):
    ...
    logging.debug('missing backup: ' +
                  str(len(missing_backup))+'/'+str(total_files))
...
def verify_tool_status(config):
    ...
    logging.debug('[FANCYTOOLS] missing files: ' +
...
class Fancytools(plugins.Plugin):
    ...
    def on_loaded(self):
        ...
        os.system(
            'sudo ln -s %s/fancytools/sys/fancytools.py /usr/local/bin/fancytools' % (FANCY_ROOT))
