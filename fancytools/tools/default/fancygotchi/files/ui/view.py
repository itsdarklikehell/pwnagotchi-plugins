...
class View(object):
    def __init__(self, config, impl, state=None):
        ...
        self._fancygotchi = fancygotchi.fancygotchi(
            self, [self._width, self._height], config
        )
        ...
        with self._lock:
            ...
            if force or len(changes):
                ...
                if th_opt["main_text_color"] != "":
                    ...
                    if color != "black":
                        self._canvas = ImageOps.colorize(
                            self._canvas.convert("L"), black=color, white="white"
                        )
                ...
                if not th_opt["stealth_mode"]:
                    ...
                    if th_opt["bg_anim_image"] != "":
                        # logging.warn(str(self._i) +'/'+str(len(self._frames)))
                        # logging.warn(th_opt['anim_web'])
                        ...
                        if th_opt["anim_web"]:
                            if isinstance(iweb, Image.Image):
                                temp_iweb = iweb.copy()
                                temp_iweb.paste(
                                    self._fancygotchi._frames[
                                        self._fancygotchi._i
                                    ].convert("RGBA")
                                )
                                iweb = temp_iweb
                            else:
                                iweb = self._fancygotchi._frames[
                                    self._fancygotchi._i
                                ].convert("RGBA")
                        else:
                            if isinstance(iweb, Image.Image):
                                ...
                                if not self._fancygotchi._frames == []:
                                    temp_iweb.paste(
                                        self._fancygotchi._frames[0].convert("RGBA")
                                    )
                                iweb = temp_iweb
                            else:
                                if not self._fancygotchi._frames == []:
                        ...
                        if th_opt["anim_display"]:
                            if isinstance(idisp, Image.Image):
                                ...
                                idisp = self._fancygotchi._frames[
                                    self._fancygotchi._i
                                ].convert("RGBA")
                        else:
                            if isinstance(idisp, Image.Image):
                    ...
                    if th_opt["bg_image"] != "":
                        if isinstance(iweb, Image.Image):
                            ...
                            if not self._fancygotchi._bg == None:
                                temp_iweb.paste(
                                    self._fancygotchi._bg, (0, 0), self._fancygotchi._bg
                                )
                            iweb = temp_iweb
                        else:
                    ...
                    if th_opt["fg_image"] != "":
                        if isinstance(iweb, Image.Image):
                            temp_iweb = iweb.copy()
                            temp_iweb.paste(
                                self._fancygotchi._fg, (0, 0), self._fancygotchi._fg
                            )
                            iweb = temp_iweb
                        else:
                ...
                if hasattr(self._fancygotchi, "_frames") and (
                    th_opt["anim_web"] or th_opt["anim_display"]
                ):
