...
class fancygotchi(object):
    ...
    def __init__(self, view, res, config):
        # logging.warn('Fancygotchi object loaded')
        self._view = view
        self._i = 0
        self._frames = []
        self._i = 0
    ...
    def theme_selector(self, size, config, boot=False):
        ...
        # logging.warn(display_path)
        # logging.warn(config['fancygotchi']['theme'])
        try:
    ...
    def fancy_change(self, partial=False, fancy_dict=[]):
        # logging.warn('Fancygotchi fancychange')
        i = 0
        out = False
        while not out:
            ...
            if not partial:
                ...
                with open("/etc/pwnagotchi/config.toml", "r") as f:
                    f_toml = toml.load(f)
                    try:
                        self._config["fancygotchi"]["rotation"] = f_toml["fancygotchi"][
                            "rotation"
                        ]
                    except:
                    ...
                    if key in [
                        ...
                        if value == "Small":
                            ...
                            if not s:
                                ...
                                self._view._state.set_font(
                                    element, fonts.status_font(fonts.Bold)
                                )
                        elif value == "BoldBig":
                            if not s:
                            ...
                            if not s:
                                ...
                                self._view._state.set_font(
                                    element, fonts.status_font(fonts.Huge)
                                )
                    elif key == "text_font":
                        if value == "Small":
                                ...
                                if value:
                                    for val in self._view._state.items():
                                        if val[0] == element:
                                            if isinstance(
                                                val[1],
                                                pwnagotchi.ui.components.LabeledValue,
                                            ):
                                                # logging.warn(element+' is a labeled value')
                                                type = self._view._state.get_attr(
                                                    element, "label"
                                                )
                                                t = "label"
                                            elif isinstance(
                                                val[1], pwnagotchi.ui.components.Text
                                            ):
                                                # if not 'face' in components[element].items():
                                                # logging.warn(element+' is a text')
                                                type = self._view._state.get_attr(
                                                    element, "value"
                                                )
                                                t = "value"
                        ...
                        if isiconic:
                            ...
                            for face_name, face_value in th_faces.items():
                                icon_path = "%simg/%s.%s" % (
                                    pwnagotchi._fancy_theme,
                                    face_name,
                                    th_img_t,
                                )
                                icon_broken = "%simg/%s.%s" % (
                                    pwnagotchi._fancy_theme,
                                    "broken",
                                    th_img_t,
                                )
