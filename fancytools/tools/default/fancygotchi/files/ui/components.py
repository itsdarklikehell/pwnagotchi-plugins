        ...
        if self.value is not None:
            if not self.icon:
                if self.wrap:
                    ...
                    text = self.value
                # logging.warn(str(self.color))
                width, height = canvas.size
                if th_opt["main_text_color"] == "":
                    imgtext = fancygotchi.text_to_rgb(
                        text, self.font, self.color, width, height
                    )
                    # logging.warn(str(self.value) + ' = ' + str(imgtext))
                ...
                if not self.f_awesome:
                    ...
                    if self.color == "white":
                        self.color = (254, 254, 254, 255)
                    if th_opt["main_text_color"] == "":
                        icon_img = ImageOps.colorize(
                            self.image.convert("L"), black=self.color, white="white"
                        )
...
class LabeledValue(Widget):
    def __init__(
        ...
        if icon:
            if not self.f_awesome:
                ...
                self.image = adjust_image(
                    icon_path, self.zoom, self.mask
    ...
    def draw(self, canvas, drawer):
        ...
        pos_value = (
            pos_label[0] + self.label_spacing + 5 * len(self.label),
            pos_label[1] - self.label_line_spacing,
        )
        if self.label is None:
            if th_opt["main_text_color"] == "":
                imgtext = fancygotchi.text_to_rgb(
                    self.value, self.label_font, self.color, width, height
                )
            ...
            if not self.icon:
                ...
                if th_opt["main_text_color"] == "":
                    imgtext = fancygotchi.text_to_rgb(
                        self.label, self.label_font, self.color, width, height
                    )
                    canvas.paste(imgtext, pos_label)
                    imgtext = fancygotchi.text_to_rgb(
                        self.value, self.text_font, self.color, width, height
                    )
                ...
                # logging.warn(self.f_awesome)
                if not self.f_awesome:
                    canvas.paste(self.image, pos_label, self.image)
                else:
                    if self.color == "white":
                        self.color = (254, 254, 254, 255)
                    # logging.warn(self.color)
                    if th_opt["main_text_color"] == "":
                        icon_img = ImageOps.colorize(
                            self.image.convert("L"), black=self.color, white="white"
                        )
