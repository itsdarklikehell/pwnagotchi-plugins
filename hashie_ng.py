...
class Hashie(plugins.Plugin):
    ...
    def on_ready(self, agent):
        ...
        logging.info(
            f"[{self.__class__.__name__}] Starting batch conversion of pcap files"
        )
        with self.lock:
    ...
    def _writePMKID(self, fullpath):
        ...
        filename = fullpath.split("/")[-1:][0].split(".")[0]
        subprocess.getoutput(
            "hcxpcapngtool -o {}.16800 {} >/dev/null 2>&1".format(
                fullpathNoExt, fullpath
            )
        )
        if os.path.isfile(fullpathNoExt + ".16800"):
        ...
        with open("/root/.incompletePcaps", "w") as isIncomplete:
            ...
            for pcapFile in lonely_pcaps:
                ...
                if os.path.isfile(fullpathNoExt + ".gps.json") or os.path.isfile(
                    fullpathNoExt + ".geo.json"
                ):
                    count += 1
            if count != 0:
                logging.info(
                    "[Hashie] Used {} GPS/GEO files to find lonely networks, "
                    "go check webgpsmap! ;)".format(str(count))
                )
            else:
                logging.info(
                    "[Hashie] Could not find any GPS/GEO files "
    ...
    def _getLocationsCSV(self, lonely_pcaps):
        ...
        if locations:
            ...
            logging.info(
                "[Hashie] Used {} GPS/GEO files to find lonely networks, "
                "load /root/locations.csv into a mapping app and go say hi!".format(
                    len(locations)
                )
            )
